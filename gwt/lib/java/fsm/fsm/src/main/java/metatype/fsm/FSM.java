...
/**
 * Defines and manages a finite state machine. Follows the UML definition for state machines regarding states, transitions, events,
 * and guard conditions. Events allow the state machine to respond to external stimuli and may cause handler invocations or state
 * transitions. Each event is handled discretely and can result in multiple state transitions. If a new event arrives while the
 * state machine is busy, it is queued and the currently processing thread will deliver the queued events in order.
 * <p>
 *
 * @see FsmEvent
 * @see FsmState
 ...
 *
 * @author metatype, dave
 *
 * @param <E>
 *           the event type
 */
public class FSM<E extends Enum<E>> {
   @Inject static private Logger LOG;

   /**
    * Defines a response to event delivered either to a state or via a state transition.
    *
    * @param <E>
    *           the event type
    */
   ...
   /**
    * Defines a guard condition applied to a state transition. The guard will allow or prevent a transition from occurring.
    *
    * @param <E>
    *           the event type
    */
   public interface Guard<E extends Enum<E>> {
       ...
       */
      boolean accept( Transition<E> t );
   }

   /**
    * Defines an event to be delivered to a state machine. Implementors may define custom fields to be consumed during event
    * handling.
    *
    * @param <E>
    *           the event type
   ...
   /**
    * Defines a transition between two states. The transition may be triggered by during event delivery if the following conditions
    * are met:
    * <ul>
    ...
    * </ul>
    *
    * When a transition is triggered the following callbacks are invoked in order (if specified):
    * <ol>
    ...
    * <ol>
    *
    * After the destination state entry is complete, it will be checked to see if any further transitions should be performed. Using
    * this approach a single event can cause the state machine to "fall through" several linked states from a single event delivery.
    *
    * @param <E>
    *           the event type
    */
   public static class Transition<E extends Enum<E>> {
      ...
      @Override public String toString() {
         return LOG.sprintf( "%s ‒ %s [%s] / %s → %s", from, trigger, guard, handler, to );
      }


      public FsmState<E> getFrom() {
         return from;
      }


      public FsmState<E> getTo() {
         return to;
      }
   ...
   /**
    * Defines a logical state. The state is configured with handlers that respond to external stimuli (events). The events are
    * delivered in order, one at a time to ensure proper synchronization of external state. The following callbacks may be
    * configured:
    * <ul>
    * <li>entry
    * <li>exit
    ...
    * </ul>
    *
    * The entry and exit callbacks are invoked during state transitions. The internal event handlers are invoked when the incoming
    * event type matches the handler mapping. If no handlers are configured for a particular type, the event is delivered to the
    * default handler.
    *
   ...
   public static class FsmState<E extends Enum<E>> {
      ...
      public FsmState( String name, Class<E> types, Command entry, Command exit, EventHandler<E> defHandler ) {
         this.name = name;
         this.entry = entry;
         this.exit = exit;
      ...
      /**
       * Returns the ordered list of state transitions. Transitions are selected by finding the first match according to the event
       * type and guard condition.
       *
      ...
      /**
       * Returns the event handlers. The appropriate handler is invoked by matching the event type when an event is delivered to a
       * state.
       *
      ...
      /**
       * Returns the default event handler. This action is invoked when an event is delivered to a state and no matching event
       * handlers are found.
       *
      ...
      protected void fireInternalHandler( FsmEvent<E> evt ) {
         EventHandler<E> eh = handlers.containsKey( evt.getType() ) ? handlers.get( evt.getType() ) : defHandler;
         if ( eh != null ) {
            eh.handleEvent( evt );
         }
      }
      ...
      protected FsmState<E> transition( FsmState<E> state, FsmEvent<E> evt ) {
         for ( Transition<E> t : state.transitions ) {
            if ( t.trigger == null || t.trigger == evt.getType() ) {
               if ( t.guard == null || t.guard.accept( t ) ) {
                  LOG.debug( "Invoking transition {}", t );
   ...
   /**
    * Defines a state that encapsulates a nested state machine. This hierarchical approach allows for factoring behavior common to
    * all the substates into the parent state. The current substate will be reset each time the parent is entered or exited.
    * <p>
   ...
   public static class FsmSubstateMachine<E extends Enum<E>> extends FsmState<E> {
      ...
      public FsmSubstateMachine( String name, Class<E> types, final FsmState<E> start, Command entry, Command exit, EventHandler<E> defHandler ) {
      ...
      @Override protected FsmState<E> handleEvent( FsmEvent<E> evt ) {
         super.fireInternalHandler( evt );

         assert current != null;
         LOG.debug( "Delivering event {} to state {}", evt, current );
      ...
      @Override protected void fireEntry() {
         super.fireEntry();
      ...
      @Override protected void fireExit() {
         current.fireExit();
         current = null;
   ...
   private final LinkedList<FsmEvent<E>> events;
   ...
   public FSM() {
      events = new LinkedList<FsmEvent<E>>();
      running = false;
   }


   /**
    * Sets the current state as required by the no-arg constructor and navigation by browser back/forward buttons.
    * @param state0
    */
   public void setCurrent( FsmState<E> state ) {
      current = state;

      LOG.debug( "About to fireEntry() on state {}", state.getName() );

      current.fireEntry();
   }


   @Inject public FSM( @Assisted FsmState<E> start ) {
      this( start, new LinkedList<FsmEvent<E>>() );
   }


   public FSM( FsmState<E> start, LinkedList<FsmEvent<E>> queue ) {
      this.events = queue;
      running = false;
      current = start;

      LOG.debug( "About to fireEntry() on state0 {}", start.getName() );
   ...
   /**
    * Delivers an event to the state machine. If the state machine is currently busy, the event will be queued and delivered in FIFO order.
    *
    * @param evt
   ...
   public void deliver( FsmEvent<E> evt ) {
      ...
      if ( running ) {
         LOG.debug( "Queued event {} for later delivery", evt );
         return;
      }
      ...
      while ( !events.isEmpty() ) {
         FsmEvent<E> next = events.removeFirst();
         LOG.debug( "Delivering event {} to state {}", next, current );
